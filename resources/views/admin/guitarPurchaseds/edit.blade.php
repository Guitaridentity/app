...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.guitarPurchased.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.guitar-purchaseds.update", [$guitarPurchased->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('owner') ? 'is-invalid' : '' }}" name="owner_id" id="owner_id">
                    @foreach($owners as $id => $entry)
                        <option value="{{ $id }}" {{ (old('owner_id') ? old('owner_id') : $guitarPurchased->owner->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('owner'))
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('purchased_date') ? 'is-invalid' : '' }}" type="text" name="purchased_date" id="purchased_date" value="{{ old('purchased_date', $guitarPurchased->purchased_date) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('purchase_price_currency') ? 'is-invalid' : '' }}" type="text" name="purchase_price_currency" id="purchase_price_currency" value="{{ old('purchase_price_currency', $guitarPurchased->purchase_price_currency) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('purchase_price_amount') ? 'is-invalid' : '' }}" type="number" name="purchase_price_amount" id="purchase_price_amount" value="{{ old('purchase_price_amount', $guitarPurchased->purchase_price_amount) }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('purchased_who') ? 'is-invalid' : '' }}" type="text" name="purchased_who" id="purchased_who" value="{{ old('purchased_who', $guitarPurchased->purchased_who) }}">
