...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.guitarchange.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.guitarchanges.update", [$guitarchange->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('guitar_owner') ? 'is-invalid' : '' }}" name="guitar_owner_id" id="guitar_owner_id">
                    @foreach($guitar_owners as $id => $entry)
                        <option value="{{ $id }}" {{ (old('guitar_owner_id') ? old('guitar_owner_id') : $guitarchange->guitar_owner->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description" id="description">{{ old('description', $guitarchange->description) }}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('date_change') ? 'is-invalid' : '' }}" type="text" name="date_change" id="date_change" value="{{ old('date_change', $guitarchange->date_change) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('done_by') ? 'is-invalid' : '' }}" type="text" name="done_by" id="done_by" value="{{ old('done_by', $guitarchange->done_by) }}">
