    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.mothers.update", [$mother->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" type="text" name="title" id="title" value="{{ old('title', $mother->title) }}">
                @if($errors->has('title'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('guitar_type') ? 'is-invalid' : '' }}" name="guitar_type_id" id="guitar_type_id">
                    @foreach($guitar_types as $id => $entry)
                        <option value="{{ $id }}" {{ (old('guitar_type_id') ? old('guitar_type_id') : $mother->guitar_type->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('guitar_brand') ? 'is-invalid' : '' }}" name="guitar_brand_id" id="guitar_brand_id">
                    @foreach($guitar_brands as $id => $entry)
                        <option value="{{ $id }}" {{ (old('guitar_brand_id') ? old('guitar_brand_id') : $mother->guitar_brand->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('guitar_brand_model') ? 'is-invalid' : '' }}" name="guitar_brand_model_id" id="guitar_brand_model_id">
                    @foreach($guitar_brand_models as $id => $entry)
                        <option value="{{ $id }}" {{ (old('guitar_brand_model_id') ? old('guitar_brand_model_id') : $mother->guitar_brand_model->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('guitar_color') ? 'is-invalid' : '' }}" name="guitar_color_id" id="guitar_color_id">
                    @foreach($guitar_colors as $id => $entry)
                        <option value="{{ $id }}" {{ (old('guitar_color_id') ? old('guitar_color_id') : $mother->guitar_color->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('strings_number') ? 'is-invalid' : '' }}" type="number" name="strings_number" id="strings_number" value="{{ old('strings_number', $mother->strings_number) }}" step="1">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('image_url') ? 'is-invalid' : '' }}" type="text" name="image_url" id="image_url" value="{{ old('image_url', $mother->image_url) }}">
