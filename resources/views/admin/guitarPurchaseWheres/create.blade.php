...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.guitarPurchaseWhere.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.guitar-purchase-wheres.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('guitar_purchase') ? 'is-invalid' : '' }}" name="guitar_purchase_id" id="guitar_purchase_id">
                    @foreach($guitar_purchases as $id => $entry)
                        <option value="{{ $id }}" {{ old('guitar_purchase_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
