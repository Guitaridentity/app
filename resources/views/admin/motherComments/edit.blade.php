...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.motherComment.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.mother-comments.update", [$motherComment->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('mother') ? 'is-invalid' : '' }}" name="mother_id" id="mother_id">
                    @foreach($mothers as $id => $entry)
                        <option value="{{ $id }}" {{ (old('mother_id') ? old('mother_id') : $motherComment->mother->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('comment') ? 'is-invalid' : '' }}" name="comment" id="comment">{{ old('comment', $motherComment->comment) }}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('signaled') ? 'is-invalid' : '' }}" type="number" name="signaled" id="signaled" value="{{ old('signaled', $motherComment->signaled) }}" step="1">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('disabled') ? 'is-invalid' : '' }}" type="number" name="disabled" id="disabled" value="{{ old('disabled', $motherComment->disabled) }}" step="1">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('likes') ? 'is-invalid' : '' }}" type="number" name="likes" id="likes" value="{{ old('likes', $motherComment->likes) }}" step="1">
                @if($errors->has('likes'))
                    <div class="invalid-feedback">
                        {{ $errors->first('likes') }}
                    </div>
